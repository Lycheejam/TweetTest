...
namespace TweetTest.Controllers {
    public class TweetController : Controller {
        ...
        public async Task<ActionResult> TweetPost(TweetViewModels tt) {
            GetToken at = await GetClaimTokens();
            var keys = MyTokens;

            var tokens = CoreTweet.Tokens.Create(keys.ConsumerKey
                                               , keys.ConsumerSecret
                                               , at.accessToken    //テーブルから参照
                                               , at.accessTokenSecret);    //テーブルから参照
            //ツイート後、レスポンス取得
        ...
        public async Task<GetToken> GetClaimTokens() {
            ...
            {
                token.accessToken = firstOrDefault.Value;
                token.accessTokenSecret = claims.FirstOrDefault(x => x.Type == "ExternalAccessTokenSecret").Value;
            }
            //コピペここまで
            return token;
        }
